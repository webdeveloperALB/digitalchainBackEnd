...
-- Updated function to handle new user signup with personalized messages
CREATE OR REPLACE FUNCTION handle_new_user()
RETURNS TRIGGER AS $$
DECLARE
  user_name TEXT;
  client_id_generated TEXT;
BEGIN
  -- Generate client ID
  client_id_generated := generate_client_id();
  ...
  -- Create personalized welcome messages for the new user
  INSERT INTO user_messages (user_id, title, content, message_type, is_read) VALUES
  (
    NEW.id,
    'Welcome to Digital Chain Bank!',
    'Hello ' || user_name || '! Your account has been successfully created with Client ID: ' || client_id_generated || '. All banking services are now available to you.',
    'success',
    FALSE
  ),
  (
    ...
    'Dear ' || user_name || ', please ensure your account information is up to date for security purposes. Your account is currently active and secure.',
    'info',
    FALSE
  ),
  (
    NEW.id,
    'Virtual Card Ready',
    'Your virtual card has been created and is ready to use. You can view your card details in the Cards section of your dashboard.',
    'info',
    FALSE
  );
