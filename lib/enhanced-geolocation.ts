...
export async function getClientIPAddress(): Promise<string> {
  try {
    // Use ipify.org as it's very reliable and fast
...
// Get location data from IP using ipapi.co (free tier: 1000 requests/day)
export async function getLocationData(ip: string): Promise<Partial<LocationData>> {
  if (!ip) {
