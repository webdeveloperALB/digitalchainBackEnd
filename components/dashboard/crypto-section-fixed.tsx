    ...
    try {
      ...
      if (user) {
        ...
        const { error } = await supabase.from("crypto_transactions").insert({
          ...
          status: "Completed",
          wallet_address: `${formData.crypto_type.toLowerCase()}1${Math.random().toString(36).substr(2, 9)}`,
        })
        ...
        setFormData({ crypto_type: "", transaction_type: "", amount: "", price_per_unit: "" })
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Cryptocurrency Trading</h2>
        <Button onClick={() => setShowTradeForm(true)} className="bg-[#F26623] hover:bg-[#E55A1F]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {cryptocurrencies.map((crypto) => (
          <Card key={crypto.code} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4 text-center">
              <Bitcoin className="w-8 h-8 mx-auto mb-2 text-[#F26623]" />
              <p className="font-medium">{crypto.name}</p>
              <p className="text-sm text-gray-600">{crypto.code}</p>
              <p className="text-lg font-bold">${crypto.price_usd.toLocaleString()}</p>
              <div className="flex items-center justify-center mt-1">
                ...
                <span className={`text-sm ${crypto.price_change_24h >= 0 ? "text-green-500" : "text-red-500"}`}>
                  {crypto.price_change_24h > 0 ? "+" : ""}
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    setFormData({
                      ...formData,
                      crypto_type: value,
                      price_per_unit: crypto?.price_usd.toString() || "",
                    })
                  }}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {cryptocurrencies.map((crypto) => (
                      <SelectItem key={crypto.code} value={crypto.code}>
                        {crypto.name} ({crypto.code}) - ${crypto.price_usd.toLocaleString()}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            {formData.amount && formData.price_per_unit && (
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-xl font-bold">
                  ${(Number.parseFloat(formData.amount) * Number.parseFloat(formData.price_per_unit)).toLocaleString()}
                </p>
              </div>
            )}

            <div className="flex gap-2">
              <Button onClick={executeTrade} className="bg-[#F26623] hover:bg-[#E55A1F]">
                Execute Trade
              </Button>
      ...
      <Card>
        ...
        <CardContent>
          {cryptoTransactions.length === 0 ? (
            <p className="text-gray-500 text-center py-4">No crypto transactions yet</p>
          ) : (
                ...
                <div key={transaction.id} className="flex justify-between items-center p-4 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Amount: {Number(transaction.amount).toLocaleString()} {transaction.crypto_type}
                    </p>
                    <p className="text-sm text-gray-600">
                      Price: ${Number(transaction.price_per_unit).toLocaleString()}
                    </p>
                    <p className="text-xs text-gray-500">{new Date(transaction.created_at).toLocaleString()}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-medium">${Number(transaction.total_value).toLocaleString()}</p>
