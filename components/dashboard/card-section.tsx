...
export default function CardSection() {
  ...
  const generateConsistentCVV = (cardId: string, cardNumber: string) => {
    // Create a simple hash from card ID and card number for consistency
    let hash = 0;
    const str = cardId + cardNumber;
    for (let i = 0; i < str.length; i++) {
      const char = str.charCodeAt(i);
      hash = (hash << 5) - hash + char;
      hash = hash & hash; // Convert to 32-bit integer
    }
    // Ensure we get a 3-digit number between 100-999
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, spending_limit: e.target.value })
                  }
                  placeholder="5000"
                />
              </div>
          ...
          cards.map((card) => (
            <div key={card.id} className="space-y-4">
              ...
              <div className="relative">
                <div className="w-80 mx-auto bg-gradient-to-r from-[#F26623] to-[#E55A1F] rounded-xl p-6 text-white shadow-lg">
                  ...
                  {showCardDetails[card.id] && (
                    <div className="border-t pt-4 space-y-2">
                      ...
                      <p className="text-sm">
                        ...
                        {generateConsistentCVV(card.id, card.card_number)}
                      </p>
                      <p className="text-sm">
